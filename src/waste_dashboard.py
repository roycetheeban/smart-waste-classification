...
def capture_camera_image():
    """Capture image from camera with toggle control"""
    # Camera toggle button
    col1, col2 = st.columns([1, 3])
    
    with col1:
        if st.session_state.camera_enabled:
            if st.button("📷 Turn Camera OFF", key="camera_off"):
                st.session_state.camera_enabled = False
                st.rerun()
        else:
            if st.button("📷 Turn Camera ON", key="camera_on"):
                st.session_state.camera_enabled = True
                st.rerun()
    
    with col2:
        status_text = "🟢 Camera Active" if st.session_state.camera_enabled else "🔴 Camera Inactive"
    ...
    if st.session_state.camera_enabled:
        ...
        st.info("Click 'Turn Camera ON' to enable camera capture")
...
def main():
    ...
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["📊 Main Dashboard", "📋 Detailed Results", "🔧 Debug Info"])
